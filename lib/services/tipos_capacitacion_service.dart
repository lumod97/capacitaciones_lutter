...
class TiposCapacitacionService {
  final String baseUrl = 'http://56.10.3.24:8000/api/capacitaciones';

  // Método para realizar una solicitud GET
  Future<List<dynamic>> getTiposCapacitacion() async {
    final response = await http.get(Uri.parse('$baseUrl/get_tipos_capacitacion'));

    if (response.statusCode == 200) {
      // Decodifica la respuesta JSON en una lista de áreas
      return jsonDecode(response.body);
    } else {
      throw Exception('Failed to load tipos capacitacion');
    }
  }
